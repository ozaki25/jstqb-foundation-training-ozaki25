...
- 「報告されたものはすべて欠陥」はひっかけで、報告されるのは不正であり、偽陽性や変更要求だと判明することもある
- 不正は SDLC のどのフェーズでも報告される可能性があり、形式は SDLC に依存する
- 欠陥マネジメントプロセスに「最低限」含まれるのはワークフローと分類のルールの2つで、関係するすべてのステークホルダーが遵守する
...
- 欠陥レポートには識別子・タイトル・日付や起票者・テスト対象と環境・故障の説明・期待結果と実際の結果・重要度と優先度・状態・参考文献などを含める（ツールを使う場合、識別子や日付など一部は自動的に含まれることがある）
