...
| 用語 | 説明 |
|------|------|
| 欠陥マネジメント（defect management） | 報告された不正を記録・解析・分類し、対応を決定してクローズまで追跡する活動 |
...
| 欠陥のライフサイクル | 欠陥レポートが報告からクローズまでにたどる状態の遷移 |

## 試験のポイント

- 報告された不正は本当の欠陥とは限らず、偽陽性や変更要求だと判明することもある
- 不正は SDLC のどのフェーズでも報告される可能性があり、形式は SDLC に依存する
- 欠陥マネジメントプロセスには最低限、不正を発見から終結まで処理するワークフローと分類のルールが含まれる
- 欠陥マネジメントプロセスは関係するすべてのステークホルダーが遵守する
- ワークフローは「記録 → 解析 → 分類 → 対応の決定 → クローズ」の流れで構成される
- 静的テスト（特に静的解析）で見つけた欠陥も同様の方法で処理することが望ましい
- 欠陥レポートの目的は「解決のための十分な情報提供」「作業成果物の品質を追跡する手段」「プロセス改善のアイデア提供」の3つ
- 重要度は影響の度合い、優先度は修正の緊急度として区別する
- 欠陥レポートには識別子・タイトル・日付や起票者・テスト対象と環境・故障の説明・期待結果と実際の結果・重要度と優先度・状態・参考文献などを含める
- ツールを使う場合、識別子や日付などの一部のデータは自動的に含まれることがある
- FL-5.5.1 は K3 なので、故障の情報をもとに欠陥レポートの項目を実際に埋められるようにしておく
