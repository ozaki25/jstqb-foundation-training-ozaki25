...
| 静的テスト（static testing） | ソフトウェアの実行を伴わないテスト。レビューと静的解析を含む |
...
| 確認テスト（confirmation testing） | 欠陥の修正によって問題が解決されたことを確認するテスト |
| リグレッションテスト（regression testing） | 修正や変更がテスト対象の他の部分に故障を発生させていないかを確認するテスト |
...
- 静的テストはソフトウェアを実行せず、レビューと静的解析を含む
