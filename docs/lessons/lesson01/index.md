...
| 用語 | 説明 |
|------|------|
| テスト（testing） | 欠陥を発見し、テスト対象の品質を評価するための一連の活動。テスト実行以外の活動も含む |
| テスト対象（test object） | テストの対象となるソフトウェアアーティファクト（要件、設計、コード、システムなど） |
| 動的テスト（dynamic testing） | ソフトウェアの実行を伴うテスト |
| 静的テスト（static testing） | ソフトウェアの実行を伴わないテスト。レビューと静的解析を含む（詳細は [lesson11](/lessons/lesson11/)） |
| 検証（verification） | 指定されている要件をシステムが満たしているかを確認すること |
| 妥当性確認（validation） | ユーザーやその他のステークホルダーのニーズを運用環境でシステムが満たしているかを確認すること |
| 欠陥（defect、バグ） | 作業成果物に含まれる不備。実行されると故障を引き起こすことがある（詳細は [lesson02](/lessons/lesson02/)） |
| 故障（failure） | コンポーネントやシステムが期待通りに動作しない事象（詳細は [lesson02](/lessons/lesson02/)） |
| デバッグ（debugging） | 故障の原因である欠陥を発見し、解析し、取り除く活動。テストとは別の活動 |
| 確認テスト（confirmation testing） | 欠陥の修正によって問題が解決されたことを確認するテスト（詳細は [lesson09](/lessons/lesson09/)） |
| リグレッションテスト（regression testing） | 修正や変更がテスト対象の他の部分に故障を発生させていないかを確認するテスト（詳細は [lesson09](/lessons/lesson09/)） |
| カバレッジ（coverage） | テスト対象をテストでどの程度網羅できたかを示す度合い |

## 試験のポイント

- テストは「テスト実行だけ」でも「検証だけに重点を置く」ものでもない（2つの誤解の否定が問われやすい）
- 検証は「指定されている要件を満たすか」、妥当性確認は「運用環境でユーザーやステークホルダーのニーズを満たすか」を確認する活動として対応づける
- テストは動的テストだけを指すのではなく、実行を伴わない静的テスト（レビューと静的解析）も含む
- 典型的なテスト目的はK1（識別）で問われるため、「欠陥を修正する」のようにリストにない活動をテスト目的と混同しない
- テストは故障や欠陥を見つける活動、デバッグは故障の原因である欠陥を発見・解析・除去する活動として区別する
- デバッグの流れは「故障の再現 → 診断 → 原因の修正」で、その後に確認テスト、続けてリグレッションテストを実施できる
- 確認テストは最初にテストした人と同じ人が担当するのがよいとされる
- 静的テストで発見した欠陥には、故障の再現や診断が不要
