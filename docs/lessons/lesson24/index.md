...
- 優先順位付けの3戦略は「実行順序の根拠」で区別し（リスクはリスク分析の結果・カバレッジは達成カバレッジ・要件は要件の優先度）、K3 なので状況の記述から戦略と最初に実行するテストケースを判断できるようにする
...
- 四象限の2軸は「ビジネス指向かテクノロジー指向か」と「チームを支援するかプロダクトを批評するか」の組み合わせで、第一象限は自動化して CI に含めるべきとされる
