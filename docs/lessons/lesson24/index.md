...
- 優先順位付けの3戦略は「実行順序の根拠」で区別し（リスクはリスク分析の結果・カバレッジは達成カバレッジ・要件は要件の優先度）、K3 なので状況の記述から戦略と実行順序を判断できるようにする（追加カバレッジの優先順位付けでは2番目以降を「最も高い追加カバレッジを達成するもの」から順に選ぶ）
- 「優先度の高いものを常に最初に実行する」はひっかけで、優先度の高いテストケースが優先度の低いテストケースに依存する場合は低い方を先に実行し、リソースの可用性も実行順序を左右する
- テストピラミッドは上層ほど粒度が粗く・分離が少なく・実行が遅いという対応を上下で入れ替えた選択肢に注意し、レイヤーの数や名称は固定ではなくモデルによって異なる点も押さえる
- 四象限の2軸は「ビジネス指向かテクノロジー指向か」と「チームを支援するかプロダクトを批評するか」の組み合わせで、使用性テストは非機能テストだが第四象限ではなく第三象限に含まれる点がひっかけになる
