...
- 4.5.1と4.5.2はK2、ATDDの適用（4.5.3）はK3で問われる
- 協働的テストアプローチの焦点は、欠陥の検出だけでなくコラボレーションとコミュニケーションによる欠陥の「回避」にある（検出を目的とするテスト技法との対比）
- ユーザーストーリーの3つのCはカード（媒体）・会話（使い方の説明）・確認（受け入れ基準）で、「確認は受け入れ基準」の対応が問われやすい
- ユーザーストーリーの共同執筆では、ビジネス・開発・テストの3つの視点を考慮する（ブレーンストーミングやマインドマップを使える）
- INVEST（独立性・交渉可能・価値がある・見積り可能・小さい・テスト可能）は頭文字と意味の対応を想起できるようにする
- ステークホルダーがユーザーストーリーをテストする方法を分からない場合、明確さや価値の反映に問題があるサインの可能性がある
- 受け入れ基準の書き方の分類はK2で、最も一般的な形式はシナリオ指向（Given/When/Then形式など）とルール指向（箇条書きの検証リスト・入出力マッピングの表形式）の2つ
- ATDDはテストファーストアプローチで、テストケースはユーザーストーリーの「実装前」にさまざまな視点のチームメンバーが作成する
- ATDDの進め方は「仕様化のワークショップ」から「テストケースの作成」の順で、ストーリーの不完全さ・曖昧さ・欠陥はワークショップ中に解決する
- テストケースの作成順序は、ポジティブテスト → ネガティブテスト → 非機能の品質特性
- テストケースの範囲のルールは3つ（ストーリーの特性をすべてカバーする・ストーリーを越えない・2つのテストケースで同じ特性を記述しない）
