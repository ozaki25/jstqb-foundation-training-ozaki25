...
| 用語 | 説明 |
|------|------|
| ユーザーストーリー（user story） | ユーザーや購入者にとって価値のある機能を表したもの。「役割・達成するゴール・ビジネス価値」の形式で書くのが最も一般的 |
| 3つのC | ユーザーストーリーの3つの重要な側面。カード（記述する媒体）・会話（使い方の説明）・確認（受け入れ基準） |
| INVEST | 優れたユーザーストーリーの基準。独立性・交渉可能・価値がある・見積り可能・小さい・テスト可能 |
| 受け入れ基準（acceptance criteria） | ユーザーストーリーの実装をステークホルダーが受け入れるために満たさなければならない条件。テスト条件とみなせる |
| シナリオ指向 | 受け入れ基準の形式の1つ。Given/When/Then形式のように振る舞いの流れで記述する |
| ルール指向 | 受け入れ基準の形式の1つ。箇条書きの検証リストや入出力マッピングの表形式で記述する |
| 受け入れテスト駆動開発（ATDD） | 受け入れ基準からテストケースを導出し、ユーザーストーリーの実装前に作成するテストファーストアプローチ |
| テストファーストアプローチ（test-first approach） | コードを書く前にテストケースを定義するアプローチ（詳細は [lesson06](/lessons/lesson06/)） |
| 実例 | 受け入れ基準に基づくテストケースが示す、ソフトウェアがどのように動作するかの例。実例とテストケースは互換的に使われる |
| 実行可能な要件（executable requirements） | 自動化された受け入れテストが果たす役割。要件をテストとして実行し、満たされているかを確認できる |

## 試験のポイント

- 協働的テストアプローチは、欠陥の検出だけでなくコラボレーションとコミュニケーションによる欠陥の回避にも焦点を当てる
- ユーザーストーリーの3つのCはカード・会話・確認で、確認は受け入れ基準に当たる
- ユーザーストーリーの共同執筆では、ビジネス・開発・テストの3つの視点を考慮する（ブレーンストーミングやマインドマップを使える）
- 優れたユーザーストーリーの基準はINVEST（独立性・交渉可能・価値がある・見積り可能・小さい・テスト可能）
- ステークホルダーがユーザーストーリーをテストする方法を分からない場合、明確さや価値の反映に問題があるサインの可能性がある
- 受け入れ基準の最も一般的な形式は、シナリオ指向（Given/When/Then形式など）とルール指向（箇条書きの検証リスト・入出力マッピングの表形式）の2つ（K2で分類が問われる）
- ATDDはテストファーストアプローチで、テストケースはユーザーストーリーの実装前にさまざまな視点のチームメンバーが作成する
- ATDDの進め方は「仕様化のワークショップ」から「テストケースの作成」の順で、ワークショップ中にストーリーの不完全さ・曖昧さ・欠陥を解決する（K3で適用が問われる）
- テストケースは最初にポジティブテスト、次にネガティブテスト、最後に非機能の品質特性の順でカバーする
- テストケースはユーザーストーリーの特性をすべてカバーし、ストーリーを越えず、2つのテストケースで同じ特性を記述しない
- テスト自動化フレームワークの形式でキャプチャした受け入れテストは、実行可能な要件になる
