...
| 根本原因（root cause） | 問題が発生する根底の理由。取り除くことで同様の故障や欠陥の防止や頻度低減につながる |
| 根本原因分析（root cause analysis） | 根本原因を特定する分析。故障の発生時や欠陥の確認時に行うのが一般的 |

## 試験のポイント

- テストは品質保証（QA）ではなく品質コントロール（QC）の形式の1つ（両者を同一視する選択肢を退ける）
- QC はプロダクト指向の是正アプローチ、QA はプロセス指向の予防的アプローチとして対応づける
...
- 欠陥には、必ず故障になるもの、特定の状況下でしか故障にならないもの、絶対に故障にならないものがある
- 故障はエラーや欠陥がなくても環境条件（放射線や電磁波など）で発生することがある
- 根本原因分析は故障の発生時や欠陥の確認時に行うのが一般的で、根本原因への対処は同様の問題の防止や頻度低減につながる
