...
- テスト技法は「比較的少ないながらも十分な」テストケースのセットを体系的に開発するのに役立ち、テスト条件の定義・カバレッジアイテムの識別・テストデータの識別を支援する
...
- 経験ベースのテスト技法は、ブラックボックスやホワイトボックスのテスト技法では見逃すような欠陥も検出でき、置き換えではなく補完の関係にある
