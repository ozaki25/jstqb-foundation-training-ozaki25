...
| 用語 | 説明 |
|------|------|
| テスト技法（test technique） | テスト分析とテスト設計を支援する手法。比較的少ないながらも十分なテストケースのセットを体系的に開発するのに役立つ |
| ブラックボックステスト技法（black-box test technique） | 仕様ベース技法。内部構造を参照せず、仕様に基づく振る舞いの分析からテストを導出する |
| ホワイトボックステスト技法（white-box test technique） | 構造ベース技法。テスト対象の内部構造や処理の分析からテストを導出する |
| 経験ベースのテスト技法（experience-based test technique） | テストケースの設計と実装に、テスト担当者の知識と経験を活用する技法 |
| テスト条件（test condition） | テストによって検証できる、テスト対象の側面。テスト技法はテスト条件の定義に役立つ |
| カバレッジアイテム（coverage item） | テスト技法を使ってテスト条件から導出する、カバレッジ測定のもとになる属性や属性の組み合わせ |

## 試験のポイント

- テスト技法は、比較的少ないながらも十分なテストケースのセットを体系的に開発するのに役立つ
- テスト技法は、テスト条件の定義・カバレッジアイテムの識別・テストデータの識別を支援する
- 3カテゴリーは導出元で区別する（ブラックボックスは仕様に基づく振る舞い、ホワイトボックスは内部構造や処理、経験ベースは知識と経験）
- ブラックボックステスト技法のテストケースは実装方法から独立しており、実装が変わっても求められる振る舞いが同じなら有用なまま
- ホワイトボックステスト技法のテストケースは設計方法に依存するため、テスト対象の設計や実装が終わってからでないと作成できない
- 経験ベースのテスト技法の有効性は、テスト担当者のスキルに大きく依存する
- 経験ベースのテスト技法は、ブラックボックスやホワイトボックスのテスト技法では見逃すような欠陥も検出でき、両者を補完する
