...
| 用語 | 説明 |
|------|------|
| テストモニタリング（test monitoring） | テストに関する情報を収集し、テストの進捗や終了基準の達成度合いを評価する活動 |
| テストコントロール（test control） | モニタリングで得た情報を用いて、ガイドと必要な補正のアクションをコントロールのための指示の形で提供する活動 |
| テスト完了（test completion） | 完了したテスト活動のデータを収集し、経験・テストウェア・その他の関連情報を統合する活動。プロジェクトのマイルストーンで発生する |
| テストメトリクス（test metrics） | 進捗状況・テスト対象の品質・テスト活動の有効性を示すために収集する測定値 |
| テスト進捗レポート（test progress report） | テストのモニタリングとコントロールの間に定期的に作成し、テストの継続的なコントロールを支援するレポート |
| テスト完了レポート（test completion report） | テスト完了時に作成し、テストの特定の段階を要約して、その後のテストのための情報を提供するレポート |
| 欠陥密度（defect density） | 欠陥メトリクスの例。成果物の規模あたりの欠陥数を示す |
| 残存リスクレベル（residual risk level） | リスクメトリクスの例。テスト後になお残っているリスクのレベル |

## 試験のポイント

- テストモニタリングは「情報の収集と進捗・終了基準の測定」、テストコントロールは「情報に基づく指示と補正のアクション」として対応づける
- コントロールのための指示の例（優先度の見直し・基準の再評価・スケジュール調整・リソース追加）を選べるようにする
- テスト完了はテストレベルの完了・イテレーションの終了・リリースなどのマイルストーンで発生する
- 代表的なメトリクスの種類（プロジェクト進捗・テスト進捗・プロダクト品質・欠陥・リスク・カバレッジ・コスト）と例の対応づけは K1 で想起できるようにする
- テスト進捗レポートは定期的に作成し、テスト完了レポートはテスト完了時に作成するというタイミングの違いを押さえる
- 進捗レポート特有の内容（次の期間で計画しているテスト）と完了レポート特有の内容（学習した教訓・未解決のリスク・修正しなかった欠陥）を区別する
- 読み手が異なればレポートに求める情報・形式的な度合い・頻度も異なる（同じチーム内へは頻繁で非形式的になりやすい）
- テストステータスの伝達手段（口頭・ダッシュボード・電子的なチャネル・オンラインドキュメント・正式なレポート）を例示できるようにする
- 分散型チームでは、より形式的なコミュニケーションが適している場合がある
