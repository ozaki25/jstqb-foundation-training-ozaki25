...
| 用語 | 説明 |
|------|------|
| 構成管理（configuration management、CM） | テスト計画書、テストケース、テスト結果記録などの作業成果物を、構成アイテムとして識別、コントロール、トラッキングするための規律 |
| 構成アイテム（configuration item） | 構成管理の対象として扱う作業成果物や構成要素。テストケース、テストスクリプト、テスト環境など |
| ベースライン（baseline） | テスト用に承認された構成アイテム。正式な変更コントロールプロセスを通じてのみ変更できる |
| バージョンコントロール（version control） | 構成アイテムのバージョンを管理し、変更を追跡すること |
| トレーサビリティ（traceability） | 作業成果物の間の関連を追跡できること（詳細は [lesson04](/lessons/lesson04/)） |

## 試験のポイント

- 構成管理の学習目標は K2「構成管理がテストをどのように支援するかを要約する」（FL-5.4.1）
- CM はテスト計画書、テストケース、テスト結果記録などの作業成果物を、構成アイテムとして識別、コントロール、トラッキングする規律を提供する
- テスト環境のような複雑な構成アイテムでは、アイテム間の関係やバージョンも記録する
- テスト用に承認された構成アイテムはベースラインとなり、正式な変更コントロールプロセスを通じてのみ変更できる
- 新しいベースラインを作成すると変更した構成アイテムの記録が残るため、以前のベースラインに戻して以前のテスト結果を再現できる
- CM が保証すること（一意な識別、バージョンコントロール、変更の追跡、他アイテムとの関連付けによるトレーサビリティの維持、ドキュメントとソフトウェアアイテムのテストドキュメントへの明記）を要約できるようにする
- 継続的インテグレーションなどの活動は通常、自動化した DevOps パイプラインの一部として実装し、その中に自動化した CM が含まれる
