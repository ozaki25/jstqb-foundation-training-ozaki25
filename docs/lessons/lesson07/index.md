...
- DevOps は開発（テストを含む）と運用が協力する「組織的アプローチ」であり、組織内の文化の転換を求める（CI/CD は DevOps が促進する「技術的な実践」で、両者の入れ替えに注意）
- テストから見た DevOps の利点（迅速なフィードバック・シフトレフトの促進・リグレッションのリスクの最小化など）と、リスクや課題（パイプラインの確立・CI/CD ツールの導入と保守・テスト自動化に必要な追加リソース）を対で要約できるようにする（K2）
...
- ふりかえりの議論の結果は記録して（通常はテスト完了レポートの一部となる）、推奨された改善点をフォローアップすることが重要。テストにとっての典型的な利点も説明できるようにする（K2）
