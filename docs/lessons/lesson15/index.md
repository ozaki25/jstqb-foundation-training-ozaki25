...
- 同値分割法は「同じパーティションの値は同等に処理される」という想定に基づき、各パーティションに対して1つのテストがあれば十分とする（K3で適用が問われる）
- パーティションは入力だけでなく、出力・構成アイテム・内部値・時間関連の値・インターフェースパラメーターについても識別できる
- パーティションは重複してはならず、空でない集合でなければならない
- 有効パーティションは有効な値を、無効パーティションは無効な値を包含し、有効・無効の定義はチームや組織によって異なる場合がある
- カバレッジ 100%には、無効パーティションを含む識別したすべてのパーティションを、テストケースで少なくとも1回カバーする必要がある
- カバレッジは「カバーしたパーティション数 ÷ 識別したパーティションの総数 × 100」で計算する（例: 5つのうち3つなら60%）
- 複数のパーティションセットがある場合の最も単純な基準はイーチチョイスカバレッジで、パーティションの組み合わせは考慮しない
