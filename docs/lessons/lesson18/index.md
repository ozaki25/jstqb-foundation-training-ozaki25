...
抽出した状態とイベントを図にします。遷移のラベルは「イベント［ガード条件］／アクション」の構文で書きます。
...
stateDiagram-v2
    ...
    [*] --> S1
    S1 --> S2 : 認証成功／マイページを表示
    S1 --> S1 : 認証失敗［失敗3回未満］／エラーを表示
    S1 --> S3 : 認証失敗［失敗3回目］／通知メールを送る
    S2 --> S1 : ログアウト
    S3 --> S1 : ロック解除
```
...
- 「認証失敗［失敗3回未満］」の遷移は、未ログインから未ログインへ戻る自己遷移です。状態は変わりませんが、エラーを表示するアクションが発生するため、遷移として数えます
- 「ログアウト」と「ロック解除」にはガード条件もアクションもないため、イベント名だけのラベルになっています
...
| 状態（state） | システムが取り得る状況。状態遷移図では丸角の箱などで表す |
| イベント（event） | 遷移のきっかけとなる出来事 |
| 遷移（transition） | ある状態から別の状態への移り変わり。イベントによって開始され、瞬時に起きることを想定する |
| ガード条件（guard condition） | イベントが遷移を引き起こすかどうかを限定する条件。省略できる |
| アクション（action） | 遷移によってソフトウェアが起こす動作。省略できる |
...
| 欠陥のマスキング（defect masking） | ある欠陥が別の欠陥の検出を妨げてしまうこと |

## 試験のポイント

- 4.2.4はK3（適用）のため、状態遷移図や状態遷移表からテストケースを導出し、カバレッジを計算する問題に備える
- 遷移のラベルは「イベント［ガード条件］／アクション」の構文で、ガード条件とアクションは省略できる
- 状態遷移表は行が状態、列がイベントで、無効な遷移を空のセルで明示的に示す（図には有効な遷移しか現れない）
- テストケースは一連のイベントとして表現し、1つのテストケースで複数の遷移をカバーできる
- 全状態カバレッジは「通過した状態の数 ÷ 状態の総数」、有効遷移カバレッジは「通した有効な遷移の数 ÷ 有効な遷移の総数」で計算する
- 有効遷移カバレッジは0スイッチカバレッジとも呼ばれ、最も広く使われているカバレッジ基準
- 有効遷移カバレッジの100%達成は全状態カバレッジの100%を保証するが、逆は成り立たない
- 全遷移カバレッジの100%には、有効な遷移をすべて通し、無効な遷移の実行も試みる
- 無効な遷移は1つのテストケースで1つだけテストし、欠陥のマスキングを避ける
- 全遷移カバレッジはミッションクリティカルおよびセーフティクリティカルソフトウェアの最低要件とすべきとされる
