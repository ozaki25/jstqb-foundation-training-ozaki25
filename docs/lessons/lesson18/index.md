...
- 自己遷移（同じ状態に戻る遷移）も1つの有効な遷移として数え、複数回通してもカバーした遷移は1種類と数える
- カバレッジ計算は分母の取り違えが定番のひっかけ（全状態は状態の総数、有効遷移は有効な遷移の総数、全遷移は有効と無効の遷移の合計で、例題では順に3・5・15）
- 無効な遷移の数は状態遷移表のセルから数える（状態数 × 列数のセルのうち空のセルが無効な遷移で、例題では15セル中10個）
- 有効遷移カバレッジは0スイッチカバレッジとも呼ばれ、最も広く使われているカバレッジ基準
- 強さの順序を押さえる（有効遷移カバレッジ100%は全状態カバレッジ100%を保証するが、逆は成り立たない）
- 全遷移カバレッジの100%には、有効な遷移をすべて通すことに加えて無効な遷移の実行も試みる
