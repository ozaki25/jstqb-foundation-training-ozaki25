...
- カバレッジ計算の勘所は分母で、ステートメントカバレッジは「実行可能な」ステートメントの総数、ブランチカバレッジは無条件・条件付きの両方を含むブランチの総数で割る
...
- ホワイトボックステストの価値は実装そのものを考慮することとコードカバレッジの客観的な測定で、実装されていない要件の欠落は検出できないためブラックボックステスト技法と組み合わせて補う
