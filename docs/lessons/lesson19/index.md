...
- 4.3.1〜4.3.3はK2（説明する）のため、2つの技法の定義・計算方法・包含関係と、ホワイトボックステストの価値を説明できるようにする
- ステートメントカバレッジの分母は「実行可能な」ステートメントの総数（「通過したステートメント数 ÷ 実行可能なステートメントの総数 × 100」）
- ブランチカバレッジは「通したブランチの数 ÷ ブランチの総数 × 100」で計算する
- ブランチには無条件（直列のコード）と条件付き（if の真偽、switch/case、ループの継続・終了）があり、100%には両方を通す
- 包含関係が最頻出（ブランチカバレッジ100%ならステートメントカバレッジも100%だが、逆は成立しない）
- else のない if は「ステートメント100%なのにブランチは未達」を作る典型例で、偽側のブランチを通すテストケースを忘れやすい
- カバレッジ100%でもすべての欠陥検出は保証されない（ステートメント100%でもゼロ除算のようなデータ依存の欠陥、ブランチ100%でも特定のパスを要する欠陥は残りうる）
- ホワイトボックステストの価値は、実装そのものを考慮するため仕様が曖昧・古い・不完全でも欠陥を検出しやすいことと、コードカバレッジを客観的に測定できること
- ホワイトボックステスト技法は動的テストだけでなく、静的テスト（コードのドライランなど）にも使用できる
- 弱点として実装されていない要件の欠落は検出できない可能性があり、ブラックボックステスト技法と組み合わせて補う
