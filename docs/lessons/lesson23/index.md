...
- 開始基準（活動のための事前条件、典型例はリソース・テストウェア・テスト対象の初期品質レベル）と終了基準（完了を宣言するために達成すべきこと、典型例は徹底度の測定と完了基準）を対応づけて見分けられるようにする（K2）
- 開始基準未達でも活動を始めること自体はできるがより困難で時間・コスト・リスクが増える一方、終了基準未達では完了を宣言できない
...
- 比率に基づく見積りと外挿（現在のプロジェクトのデータを使う）はメトリクスベース、ワイドバンドデルファイ（プランニングポーカーはそのバリエーション）と三点見積りは専門家の経験に基づく技法として分類する
