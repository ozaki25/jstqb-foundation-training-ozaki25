...
- 満たされない場合の帰結の違いに注意する（開始基準未達でも活動を始めること自体はできるが、より困難で時間・コスト・リスクが増える一方、終了基準未達では完了を宣言できない）
- 開始基準と終了基準は「プロジェクトで1つ」ではなく、テストレベルごとに定義するべきで、テスト対象によっても異なる
...
- アジャイルでは終了基準が完了（done）の定義、ユーザーストーリーの開始基準が準備完了（Ready）の定義（対応を入れ替えた選択肢に注意）
- 見積りは多くの仮定に基づくため常に誤差の可能性があり関係者への明示が重要である点と、大きなタスクは小さなタスクに分解してから見積もると正確になる点は、見積りの前提として問われる
- 比率に基づく見積りと外挿はメトリクスベース、ワイドバンドデルファイと三点見積りは専門家の経験に基づく技法として分類する（外挿は過去ではなく現在のプロジェクトのデータを使い、イテレーティブな SDLC に非常に適する）
- プランニングポーカーは独立した技法ではなく、ワイドバンドデルファイのバリエーション
- 比率3:2と開発工数600人日からテスト工数400人日を導くような計算はK3で問われる（比率の向きの取り違えが典型的なミス）
- 三点見積りは `E = (a + 4m + b) / 6` と `SD = (b - a) / 6` で計算し、結果を範囲（E±SD）で答えられるようにする（`m` だけを4倍・分母は6）
