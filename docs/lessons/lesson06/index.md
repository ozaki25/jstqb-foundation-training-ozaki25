...
| 用語 | 説明 |
|------|------|
| SDLCモデル（software development lifecycle model） | ソフトウェア開発プロセスを抽象的かつハイレベルに表現したもの。開発フェーズや活動の種類の論理的・時系列的な関連を定義する |
| シーケンシャル開発モデル（sequential development model） | 開発フェーズを順に進めるSDLCモデル。ウォーターフォールモデルやV字モデルが例 |
| イテレーティブ開発モデル（iterative development model） | 開発を繰り返しのサイクルで進めるSDLCモデル。スパイラルモデルやプロトタイピングが例 |
| インクリメンタル開発モデル（incremental development model） | システムを増分として少しずつ積み上げるSDLCモデル。統一プロセスが例 |
| テストファーストアプローチ（test-first approach） | コードを書く前にテストケースを定義するアプローチ。TDD・ATDD・BDDが該当する |
| TDD（テスト駆動開発） | テストケースを通じてコーディングを導くアプローチ。テストを書き、テストを満たすコードを書き、リファクタリングする |
| ATDD（受け入れテスト駆動開発） | 受け入れ基準からテストを導き出し、開発前にテストを書くアプローチ（詳細は [lesson21](/lessons/lesson21/)） |
| BDD（振る舞い駆動開発） | 望ましい振る舞いを自然言語のテストケースで表現するアプローチ。通常はGiven/When/Then形式を使う |
| リグレッションテスト（regression testing） | 変更がテスト対象の他の部分に故障を発生させていないかを確認するテスト（詳細は [lesson09](/lessons/lesson09/)） |

## 試験のポイント

- SDLCの選択が影響を与える事柄（テスト活動の範囲とタイミング・テストドキュメントの詳細レベル・テスト技法とテストアプローチの選択・テスト自動化の範囲・テスト担当者の役割と責任）を説明できるようにする
- シーケンシャル開発モデルの初期段階でテスト担当者が参加するのは、要件レビュー・テスト分析・テスト設計
- シーケンシャル開発モデルでは実行可能なコードが後のフェーズで作成されるため、動的テストをSDLCの初期には実施できない
- イテレーティブ・インクリメンタル開発モデルでは、各イテレーションで静的テストと動的テストの両方をすべてのテストレベルで実行する可能性がある
- インクリメントの頻繁な提供には、迅速なフィードバックと広範なリグレッションテストが必要
- アジャイルプロジェクトでは、文書化の軽量化・テスト自動化の充実・経験ベースのテスト技法による手動テストが好まれる
...
- テストファーストアプローチでは、将来の適応やリファクタリングに備えてテストケースを自動テストとして持続させることがある
