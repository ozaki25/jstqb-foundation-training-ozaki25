...
| 不正（anomaly） | レビューや静的解析で識別される、欠陥の可能性がある問題。分析と議論を経て欠陥かどうかを判断する |
| モデレーター（moderator） | レビューミーティングを効果的に運営する役割。テクニカルレビューを主導する（詳細は [lesson12](/lessons/lesson12/)） |
| レビューリーダー（review leader） | レビューの全体的な責任を負う役割。インスペクションでは作成者は担当できない（詳細は [lesson12](/lessons/lesson12/)） |

## 試験のポイント

- 形式度は「非形式的レビュー → ウォークスルー → テクニカルレビュー → インスペクション」の順に高くなる
- 作成者が主導するのはウォークスルーで、テクニカルレビューは技術的に適格なレビューアが実施しモデレーターが主導する
- 非形式的レビューは定義されたプロセスに従わず、正式にドキュメント化されたアウトプットも必要とせず、主な目的は不正の検出
- インスペクションは汎用のレビュープロセスに完全に従い、主な目的は不正を最大数発見すること
- インスペクションではメトリクスを収集して SDLC の改善に使用し、作成者はレビューリーダーや書記を担当できない
...
- 成功要因は K1 で問われるため、9つの内容（明確な目的と測定可能な終了基準・適切な種別の選択・小さく分割・フィードバック・十分な準備時間・マネジメントの支援・組織文化への定着・トレーニング・ミーティングのファシリテート）を想起できるようにする
- 参加者の評価は決してレビューの目的にしてはならない
