...
- 1つの種別に固有の特徴はひっかけに使われやすい（レビューアの教育を目的に挙げるのはウォークスルーのみ、メトリクスによるプロセス改善はインスペクションのみで、インスペクションでは作成者がレビューリーダーや書記を担当できない）
- 成功要因はK1（想起する）で問われるため、本文の9つの要因を漏れなく想起できるようにする
