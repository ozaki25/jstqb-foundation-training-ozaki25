...
- 1つの種別に固有の特徴はひっかけに使われやすい（作成者が主導しレビューアの教育を目的にできるのはウォークスルーのみ、メトリクスによるプロセス改善はインスペクションのみで、インスペクションでは作成者がレビューリーダーや書記を担当できない）
- 成功要因はK1（想起する）で問われるため本文の9つの要因を漏れなく想起できるようにし、参加者の評価は決してレビューの目的にしてはならない（頻出のひっかけ）
