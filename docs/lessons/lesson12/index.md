...
- 早期かつ頻繁なフィードバックの利点はK1（識別する）で問われる（品質問題の早期伝達・誤解の防止・変更の早期理解と実装・チームの理解向上・価値とリスクに効くフィーチャーへの集中）
...
- 不正・推奨・質問の識別と記録は個々人のレビュー、不正のステータス・オーナー・アクションの決定は共有と分析という分担もひっかけどころ
- 不正は必ずしも欠陥ではなく、欠陥かどうかは共有と分析で議論して判断する
- サイズの大きい作業成果物では、全体のレビューを完了させるためにレビュープロセスを2〜3回行うことがある
- 6つの役割と責務の対応はK1（想起する）で問われる
- マネージャー（何をレビューするかの決定とリソースの提供）とレビューリーダー（参加者と日時・場所の決定、全体的な責任）の取り違えが定番のひっかけ
- モデレーター（ミーティングの運営と安全な環境づくり）と書記（不正の取りまとめとレビュー情報の記録）の分担も対で押さえる
