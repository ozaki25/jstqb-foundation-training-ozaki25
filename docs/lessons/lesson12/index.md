...
| 用語 | 説明 |
|------|------|
| レビュー（review） | 作業成果物を評価してフィードバックを得る、静的テストの一種（[lesson11](/lessons/lesson11/)） |
| 不正（anomaly） | 期待から逸脱していると識別された事項。分析と議論の結果、欠陥と判断されるとは限らない |
| マネージャー（manager） | 何をレビューするかを決定し、人員や時間などのリソースを提供する役割 |
| 作成者（author） | レビュー対象の作業成果物を作成し、修正する役割 |
| モデレーター（moderator） | レビューミーティングを効果的に運営する役割。ファシリテーターとも呼ばれる |
| 書記（scribe） | 不正の取りまとめとレビュー情報の記録を担う役割。記録係とも呼ばれる |
| レビューア（reviewer） | 作業成果物の品質を精査し、レビューを実施する役割 |
| レビューリーダー（review leader） | 参加者と開催の日時・場所を決め、レビューの全体的な責任を負う役割 |
| チェックリストベースドレビュー（checklist-based reviewing） | あらかじめ用意した項目のリストを手がかりに作業成果物を精査するレビュー技法 |
| シナリオベースドレビュー（scenario-based reviewing） | 想定するシナリオに沿って作業成果物を精査するレビュー技法 |

## 試験のポイント

- 早期かつ頻繁なフィードバックの利点はK1で問われる（品質問題の早期伝達・誤解の防止・変更の早期理解と実装・チームの理解向上・価値とリスクに効くフィーチャーへの集中）
- フィードバックが少ない場合のリスク（コストのかかる手直し・納期の遅れ・責任のなすり合い・プロジェクトの失敗）も利点と対で押さえる
- レビュープロセスの5つの活動を順序どおりに要約できるようにする（計画 → レビューの立ち上げ → 個々人のレビュー → 共有と分析 → 修正と報告）
- レビューの範囲（目的・対象の作業成果物・品質特性・重点領域・終了基準・裏付け情報・工数と時間枠）の定義は計画で行う
- 関係者や物事の準備ができたことの明確化は、レビューの立ち上げのゴール
- 不正・推奨・質問の識別と記録は個々人のレビューで行い、チェックリストベースドレビューなどのレビュー技法を適用する
- 不正は必ずしも欠陥ではないため、共有と分析ですべての不正のステータス・オーナー・必要なアクションを決める
- 欠陥レポートの作成とレビュー結果の報告は修正と報告で行い、終了基準に達すれば作業成果物を受け入れられる
- サイズの大きい作業成果物では、レビュープロセスを2〜3回行うことがある
- 6つの役割と責務の対応はK1で問われる（特にマネージャーとレビューリーダー、モデレーターと書記の取り違えに注意）
- モデレーターとファシリテーター、書記と記録係は、それぞれ同じ役割の別名
