...
- FL-6.2.1 は K1（記憶）であり、利点とリスクの一覧にある項目とない項目を見分けられるようにする
- ツールを取得するだけでは成功は保証されず、導入・メンテナンス・トレーニングの労力が必要になる
- 利点は「反復作業の削減と時間の節約」「一貫性・再実行性の向上によるヒューマンエラー防止」「評価の客観性の向上」「テスト情報へのアクセスの容易化」「テスト実行時間の短縮」「新しいテストを設計する時間の増加」の6つ
- リスクは「非現実的な期待」「時間・コスト・労力の見積りの誤り」「手動テストが適切な場面でのツール使用」「ツールへの過剰な依存」「ベンダーの廃業・売却・サポート不足」「オープンソースの停滞」「開発プラットフォームとの非互換」「規制要件や安全基準に適合しない不適切なツール選択」
- 「ツールへの過剰な依存」の例は、人の批判的思考の必要性を無視すること
- テスト実行時間の短縮は、欠陥の早期発見・迅速なフィードバック・市場投入までの時間の短縮につながる
