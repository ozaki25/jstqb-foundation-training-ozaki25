...
| 作業成果物（work product） | 開発やテストの過程で作成される成果物。要件仕様書・ソースコード・テスト計画書・契約書・モデルなど |
| 早期テストの原則 | テストを早期に開始するほど時間とコストを節約できるという、テストの7原則の1つ（詳細は [lesson03](/lessons/lesson03/)） |

## 試験のポイント

- 静的テストはソフトウェアを実行せず、レビュー（人手）または静的解析（ツール）で作業成果物を評価する
- 要件仕様書・ソースコード・テスト計画書・契約書・モデルなど、ほとんどの作業成果物は静的テストで確認できる（K1で種類を認識する）
- 読んで理解できる成果物はレビューの対象になりうるが、静的解析には構造を持つ成果物が必要という違いを押さえる
- 人間による解釈が困難なものや、法規制がある第三者の実行可能コードのようにツールで解析してはいけないものは静的テストに適さない
- 静的テストは SDLC の初期段階で欠陥を検出でき、早期テストの原則を満たす
- 到達できないコードや実行不可能な作業成果物に内在する欠陥は、動的テストでは検出できない
- レビューの実装にはコストがかかるが、後半の欠陥修正が減るため通常はプロジェクト全体のコストを下げる
- 静的テストは欠陥を直接発見し、動的テストは故障を引き起こして分析により欠陥を特定するという対比が最重要
- 静的テストは実行不可能な成果物にも適用でき、動的テストは実行可能な成果物にしか適用できない
- 保守性のような実行に依存しない品質特性は静的テスト、性能効率性のような実行に依存する品質特性は動的テストで測定できる
- 要件の矛盾・曖昧さ・欠落、標準からの逸脱、バッファオーバーフローのような特定のセキュリティ脆弱性は、静的テストが早期かつ安価に検出できる欠陥の例
