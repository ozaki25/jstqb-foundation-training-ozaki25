...
- 静的テストと動的テストの比較・対比はK2の頻出テーマ（「欠陥を直接発見する」のが静的テスト、「故障を引き起こして分析で欠陥を特定する」のが動的テストで、静的テストは実行不可能な成果物にも適用できる）
- 品質特性との対応はひっかけどころ（保守性のような実行に依存しない特性は静的テスト、性能効率性のような実行に依存する特性は動的テストで測定する）
- レビューは読んで理解できるあらゆる成果物を対象にできるが、静的解析にはチェックするための構造を持つ成果物が必要（テストケースは不要で、典型的にはツールを使用する）
...
- 静的テストの価値はK2で問われる（SDLC初期の欠陥検出・ステークホルダー間の共通理解・コミュニケーションの改善）
