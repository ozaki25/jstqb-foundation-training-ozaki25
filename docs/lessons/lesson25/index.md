...
| リスクレベル（risk level） | リスクの尺度。リスクの可能性（likelihood）とリスクの影響（impact）の2つの要素で表す |
...
- リスクレベルはリスクの可能性（顕在化する確率）とリスクの影響（顕在化した結果の損害）の2つの要素で表し、リスクの可能性は0より大きく1より小さい
- プロジェクトリスク（納期遅れ・スキル不足・スコープクリープ・サプライヤーの倒産など）とプロダクトリスク（機能の誤り・不十分な応答時間・セキュリティ脆弱性など）を例で区別できるようにする（K2）
...
- テストによるリスク軽減の行動と、リスクモニタリングの3つの目的（軽減措置の効果確認・アセスメント改善のための情報入手・新たなリスクの識別）は、本文の一覧から見分けられるようにする
