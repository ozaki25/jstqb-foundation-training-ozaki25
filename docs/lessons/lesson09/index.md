...
| ISO/IEC 25010 | 非機能ソフトウェア品質特性を分類している標準。性能効率性・互換性・使用性・信頼性・セキュリティ・保守性・移植性を挙げている |
...
| 影響度分析（impact analysis） | ソフトウェアのどの部分が変更の影響を受けるかを示す分析。リグレッションテストの範囲の最適化に役立つ |

## 試験のポイント

- 機能テストは「何をするか」、非機能テストは「どのようにうまく振る舞うか」を評価する対応づけで区別する（K2）
- 非機能テストの主な目的は、ISO/IEC 25010 が分類する非機能ソフトウェア品質特性（性能効率性・互換性・使用性・信頼性・セキュリティ・保守性・移植性）のチェック
...
- 確認テストは「元の欠陥が正常に修正されたこと」、リグレッションテストは「変更による悪影響がないこと」を確認する活動として区別する
- 時間や費用が限られている場合、確認テストは故障を再現する手順の実行だけに限定することがある
- リグレッションテストの悪影響は、変更したコンポーネント・同じシステム内の他のコンポーネント・接続されている他システム・環境にまで及びうる
- リグレッションテストの範囲の最適化には影響度分析が役立つ
- リグレッションテストスイートは何度も実行し拡充されるため自動化の有力な候補であり、自動化は早期に開始すべきとされる
- あるテストレベルで修正や変更があった場合、確認テストとリグレッションテストはすべてのテストレベルで必要になる
